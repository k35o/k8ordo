...
export const description = message({
  ja: 'アプリケーションのカラースキーム軸を持つ。訪問者の設定（light / dark / 未設定＝システム追従）を localStorage に置き、システムの設定と突き合わせて `<html>` の `dark` クラスに解決する。ルートレイアウトに置く 1 つの Provider が、最初の描画の前に付けるインラインスクリプトの描画も、hydrate 後の追従も担い、hook はそれを読むだけ。',
  en: 'Owns the colour-scheme axis of an application. The visitor’s preference (light, dark, or nothing, which follows the system) lives in localStorage, is resolved against the system, and becomes the `dark` class on `<html>`. One provider in the root layout renders the inline script that puts it there before the first paint and keeps it there after hydration; a hook reads it.',
});
...
export const featureSystem = message({
  ja: '未設定はシステムに追従',
  en: 'Absence follows the system',
});

export const featureSystemDescription = message({
  ja: '保存されるのは訪問者が選んだときだけ。選んでいなければ Provider の `defaultPreference`（既定は `system`）に従い、`prefers-color-scheme` の変化にその場で追従する。初回に見たシステムの値に固定されない。',
  en: 'A preference is stored only when the visitor chose one. Otherwise the provider’s `defaultPreference` applies (`system` unless told otherwise), following `prefers-color-scheme` as it changes — never pinned to what the system said on the first visit.',
});
...
export const featureStateDescription = message({
  ja: "`defineLocalState('color-scheme')` の 1 行が保存先。キーも JSON の形もここには書かれず、タブ間の同期も古い行のサルベージも state のもの。",
  en: "One `defineLocalState('color-scheme')` is where it lives. The key and the JSON envelope are never spelled here; cross-tab sync and salvage of an old row are state’s.",
});
...
export const demoDescription = message({
  ja: 'ヘッダーの切替ボタンも、このページの選択肢も、同じ `useColorScheme()` です。「システム」に戻すと保存行が消え、OS の設定に追従します。',
  en: 'The switcher in the header and the choices below are the same `useColorScheme()`. Choosing “system” removes the stored row and follows the OS again.',
});
...
export const exampleDescription = message({
  ja: 'ルートレイアウトに Provider を 1 つ、切替に hook を 1 つ。それ以外に書くものはありません。',
  en: 'One provider in the root layout, one hook in the switcher. There is nothing else to write.',
});
