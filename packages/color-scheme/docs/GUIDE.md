...
on `<html>`, which `@k8ordo/ui`'s tokens and Tailwind's own dark variant
read. One provider does all of it: it renders the inline script that puts
the class there before the first paint, keeps it right after hydration, and
hands a hook what it reads.
...
- **The colours.** Which colour `dark` means is the stylesheet's business —
  `@k8ordo/ui`'s tokens, or any other that reads the class.
- **Where the preference lives.** That is `@k8ordo/state`'s
  `defineLocalState`; this package declares one and reads it, so the key
  and the JSON envelope are never spelled here.
- **A guess on the server.** No cookie, no header: a server render shows
  the provider's default, and the inline script is what makes the first
  paint right.
...
| `setPreference` | Stores a preference; `'system'` stores none and follows the system again. |
...
- **No flash.** The script runs before the first paint and reads the same
  row the provider writes; a dark page loads dark.
- **Nothing chosen follows the default, and the default follows the
  system.** A visitor who never chose is never pinned to what the system
  said on their first visit, and a row an older version wrote, or one that
  is not JSON, reads as nothing chosen.
- **The server renders the default**, and the provider corrects itself on
  hydration without touching the document: the render that hydrates reads
  the server's guesses and writes nothing, and the render that follows reads
  the store and writes what the script already put there. Until then the
  HTML shows the default — a switcher's icon, say — which is the one thing
  no server can know; a component that must not show it even for a moment
  says so with React's `use(browser())` under a `<Suspense>`.
- **Tabs agree.** The preference propagates across tabs through the
  `storage` event, as any `@k8ordo/state` local state does.
...
A client-only mount also runs the script the provider renders — a script
element React creates executes — which is idempotent with the effect.
