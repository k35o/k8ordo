...
it('許可すると検索が走り、出典と生成 UI 付きで答える', async () => {
  ...
  await screen.getByRole('button', { name: '許可' }).click();
...
it('拒否すると検索せずに答える', async () => {
  ...
  await screen.getByRole('button', { name: '拒否' }).click();

  await expect
    .element(screen.getByText(/^検索せずに答えます。/u))
    .toBeVisible();
  await expect
    .element(screen.getByRole('list', { name: '出典' }))
...
it('添付したファイルが吹き出しに出て、モデルにも届く', async () => {
  ...
  await expect
    .element(screen.getByRole('list', { name: '添付ファイル' }))
    .toBeVisible();
...
it('再生成すると同じ質問にもう一度答え直す', async () => {
  ...
  await screen.getByRole('button', { name: '良い回答' }).click();
  await expect
    .element(screen.getByRole('button', { name: '良い回答' }))
    .toHaveAttribute('aria-pressed', 'true');

  await screen.getByRole('button', { name: '再生成' }).click();
