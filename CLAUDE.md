...
4. Docs go under `/<name>/…` on the site: a landing, then a guide that starts at `/<name>/get-started` and has a page per topic, not a single page restating the README. Only `/` is shared, plus `/llms.txt`, the one index across every package. The site also serves the shipped `docs/` verbatim as markdown twins at `/<name>/docs/…`, which takes three registrations: an entry in `MAPPINGS` (`apps/docs/scripts/copy-reference-docs.ts`), `apps/docs/public/<name>/` in the root `.gitignore` (the copies are never committed), and an entry in `SOURCES` (`apps/docs/scripts/generate-llms-txt.ts`), after which `pnpm build` rewrites the committed `apps/docs/public/llms.txt`. Neither script finds packages on its own, so a missing entry fails nothing — the docs just go unserved. `@k8ordo/ui` is the one exception to the layout: its twins stay at `/docs/…` and its design spec at `/design.md`, because they predate the package-first rule and links to them are already published. See `apps/docs/CLAUDE.md`.
